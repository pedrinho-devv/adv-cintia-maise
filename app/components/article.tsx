...
export default function ContentArea() {
  ...
  useEffect(() => {
    async function fetchData() {
      const query = `*[_type == "post"][0...4] | order(_createdAt desc) {
        _id,
        title,
        "slug": slug.current,
      ...
      console.log(data);  // Verifique se os dados são recuperados corretamente
      setArticles(data);
    }
  ...
  return (
    <section id="blog" className="bg-[#C3A039] p-6 sm:p-10">
      <div className="flex justify-between items-center mb-6">
        ...
        <Link href="/artigos" className="text-white text-sm hover:underline flex items-center">
          Ver Mais →
        </Link>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
        {articles.map((article, index) => (
          <Link key={index} href={`/artigos/${article.slug}`} className="relative overflow-hidden rounded-lg group cursor-pointer">
